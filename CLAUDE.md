...
- Skills that coordinate multiple agents (`claude-codex-workflow`, `herdr-workflow`) spell out an explicit **Role Contract** (who edits files vs. who only reviews) and an **Autonomy Contract** (what the agent may decide on its own vs. what must be escalated to the user). Follow this pattern for any new multi-agent or loop-style skill rather than leaving autonomy boundaries implicit.
